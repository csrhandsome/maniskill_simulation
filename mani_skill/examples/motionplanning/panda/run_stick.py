...
from mani_skill.examples.motionplanning.panda.solutions import solvePushT
# 这里的SOLUTIONS也需要用到修改过后的PullCubeEnv,
# 所以之前在keyboard修改过了之后这里就不用修改了
MP_SOLUTIONS = {
    "PushT-v1": solvePushT
}
def parse_args(args=None):
    parser = argparse.ArgumentParser()
    parser.add_argument("-e", "--env-id", type=str, default="PushT-v1", help=f"Environment to run motion planning solver on. Available options are {list(MP_SOLUTIONS.keys())}")
    ...
    parser.add_argument("--robot-uid", type=str, default="panda_stick", help="Robot UID, using panda_stick for PandaStick robot")
...
def _main(args, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    if env_id not in MP_SOLUTIONS:
        raise RuntimeError(f"No already written motion planning solutions for {env_id} with PandaStick. Available options are {list(MP_SOLUTIONS.keys())}")
    ...
    env = RecordEpisode(
        env,
        output_dir=osp.join(args.record_dir, f"{env_id}", "motionplanning"),
        trajectory_name=new_traj_name, save_video=args.save_video,
        source_type="motionplanning",
        source_desc=f"PandaStick motion planning solution for {temp_env_id}",
        video_fps=30,
        record_reward=False,
        save_on_reset=False
    )
    ...
    print(f"Motion Planning Running on {env_id} with {args.robot_uid}")
    ...
    while True:
        try:
            res = solve(env, seed=seed, debug=False, vis=True if args.vis else False, use_stick_planner=True)
